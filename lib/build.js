...
module.exports = function () {
    ...
    try {
        ...
        blogmoreJSON = fs.readFileSync(path.join(CWD, 'assets', 'init.json'), 'utf8');
    } catch (err) {
    ...
    var postDest = path.join(dest, 'post');
    Promise
    .resolve(dest)
    .then(util.mkdir)
    ...
    .then(function (files) {
        return Promise.all(files.map(function (file) {
            console.log('building: ' + file.name.yellow);
            return util.readFile(file.path).then(function (content) {
                ...
                makeBlogDB(path.join('post', title + '.html'), title, contentObj.date, contentObj.categories, contentObj.tags);
    ...
    /*.then(function () {
        return util.copy(path.join(__dirname, '../assets', 'components'), path.join(dest, 'components'));
    })*/
    .then(function () {
